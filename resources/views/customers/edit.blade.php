    ...
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white form-container shadow-2xl rounded-3xl p-10 max-w-md mx-auto">
            ...
            <div class="text-center mb-8">
                <h1 class="page-title text-3xl font-bold mb-2 text-purple-700">Edit Customer</h1>
                <p class="text-gray-500 text-sm">Update customer information</p>
            </div>

            <form action="{{ route('customers.update', $customer) }}" method="POST" class="space-y-6" id="customerForm">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    <div class="flex items-center space-x-3">
                        ...
                        <span id="statusBadge" class="{{ old('status', $customer->status) == 'active' ? 'status-active' : 'status-inactive' }} px-3 py-1 rounded-full text-white text-sm font-medium">
                            {{ old('status', $customer->status) == 'active' ? 'Active' : 'Inactive' }}
                        </span>
                    </div>
                </div>
                ...
                <div class="bg-gray-50 rounded-lg p-4 mb-6">
                    ...
                    <div class="text-xs text-gray-500 space-y-1">
                        <p><span class="font-medium">Created:</span> {{ $customer->created_at->format('M d, Y H:i') }}</p>
                        <p><span class="font-medium">Last Updated:</span> {{ $customer->updated_at->format('M d, Y H:i') }}</p>
                        <p><span class="font-medium">Customer ID:</span> #{{ $customer->id }}</p>
                    </div>
                </div>
            ...
            {{-- Hidden Delete Form --}}
            <form id="deleteForm" action="{{ route('customers.destroy', $customer) }}" method="POST" class="hidden">
                @csrf
                @method('DELETE')
            </form>
        </div>
