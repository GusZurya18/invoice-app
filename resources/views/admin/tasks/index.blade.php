...
@section('content')
    <div class="min-h-screen rounded-xl bg-gradient-to-br from-indigo-100 via-blue-50 to-pink-100 py-4 md:py-8">
                        ...
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-gray-600">
                                Menampilkan hasil pencarian untuk: <span
                                    class="font-semibold text-gray-900">"{{ request('search') }}"</span>
                            </span>
                            <a href="?{{ request('status') ? 'status=' . request('status') : '' }}"
            ...
            <div class="bg-white shadow-sm rounded-2xl overflow-hidden">
                                    ...
                                    <tr class="hover:bg-gray-50 transition duration-150">
                                        <td class="px-6 py-4">
                                            <input type="checkbox"
                                                ...
                                                value="{{ $task->id }}" onclick="updateBulkDeleteButton()">
                                        </td>
                                        <td class="px-6 py-4">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900">
                                                {{ $task->assignee->name ?? 'Belum ditugaskan' }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            @if ($task->status == 'done')
                                                <span
                                                    class="inline-flex items-center px-3 py-1 rounded-lg text-xs font-medium bg-green-100 text-green-700">
                                            ...
                                            @elseif($task->status == 'pending')
                                                <span
                                                    class="inline-flex items-center px-3 py-1 rounded-lg text-xs font-medium bg-orange-100 text-orange-700">
                                                    <svg class="w-3.5 h-3.5 mr-1.5" fill="currentColor"
                                                        viewBox="0 0 20 20">
                                                        <path fill-rule="evenodd"
                                            ...
                                            @elseif($task->status == 'in_progress')
                                                <span
                                                    class="inline-flex items-center px-3 py-1 rounded-lg text-xs font-medium bg-blue-100 text-blue-700">
                                                    <svg class="w-3.5 h-3.5 mr-1.5" fill="currentColor"
                                                        viewBox="0 0 20 20">
                                                        <path fill-rule="evenodd"
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <div class="grid grid-cols-2 gap-2 mb-3 text-xs">
                                            <div>
                                                ...
                                                <span
                                                    class="text-gray-900 font-medium ml-1">{{ Str::limit($task->assignee->name ?? 'Belum ditugaskan', 15) }}</span>
                                            </div>
                                            <div>
                                                <span class="text-gray-500">Priority:</span>
                                                <span
                                                    class="ml-1 px-2 py-0.5 inline-flex text-xs font-semibold rounded-full
                                                @if ($task->priority == 'high') bg-red-100 text-red-800
                                                @elseif($task->priority == 'medium') bg-yellow-100 text-yellow-800
                                                @else bg-green-100 text-green-800 @endif">
                                                    {{ ucfirst($task->priority) }}
                                            ...
                                            <div>
                                                ...
                                                <span
                                                    class="text-gray-900 ml-1">{{ $task->start_date ? $task->start_date->format('d/m/Y') : '-' }}</span>
                                            </div>
                                            <div>
                                                <span class="text-gray-500">Deadline:</span>
                                                <span
                                                    class="text-gray-900 ml-1">{{ $task->deadline ? $task->deadline->format('d/m/Y') : '-' }}</span>
                                            </div>
                                        </div>
                                        ...
                                        <div class="flex items-center justify-between">
                                            <div>
                                                ...
                                                @elseif($task->status == 'pending')
                                                    <span
                                                        class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium bg-orange-100 text-orange-700">
                                                        <svg class="w-3 h-3 mr-1" fill="currentColor"
                                                            viewBox="0 0 20 20">
                                                            <path fill-rule="evenodd"
                                                                d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z"
                                                ...
                                                @elseif($task->status == 'in_progress')
                                                    <span
                                                        class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium bg-blue-100 text-blue-700">
                                                        <svg class="w-3 h-3 mr-1" fill="currentColor"
                                                            viewBox="0 0 20 20">
                                                            <path fill-rule="evenodd"
                                                                d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-11a1 1 0 10-2 0v3.586L7.707 9.293a1 1 0 00-1.414 1.414l3 3a1 1 0 001.414 0l3-3a1 1 0 00-1.414-1.414L11 10.586V7z"
                ...
                @else
                    ...
                    <div class="text-center py-12 md:py-16 px-6">
                        @if (request('search'))
                            <svg class="mx-auto h-16 md:h-24 w-16 md:w-24 text-gray-300" fill="none"
                                stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                            </svg>
                            <h3 class="mt-4 text-base md:text-lg font-semibold text-gray-900">Tidak ada hasil yang
                                ditemukan</h3>
                            <p class="mt-2 text-sm text-gray-500">Kami tidak dapat menemukan task dengan kata kunci "<span
                                    class="font-semibold">{{ request('search') }}</span>"</p>
