        ...
        <div class="max-w-3xl mx-auto">
            ...
            <div class="bg-white rounded-3xl shadow-2xl overflow-hidden">
                ...
                <div class="px-8 pt-8 pb-6">
                    ...
                    <h1 class="text-3xl font-bold text-blue-600 text-center">Edit Task</h1>
                </div>
                ...
                <form action="{{ route('admin.tasks.update', $task) }}" method="POST" id="taskForm" class="px-8 pb-8">
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" name="title" id="title" placeholder="Enter Task Title"
                            class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 @error('title') border-red-500 @enderror"
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="description" id="description" rows="4" placeholder="Enter Task Description"
                            class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 resize-none @error('description') border-red-500 @enderror">{{ old('description', $task->description) }}</textarea>
                        @error('description')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="relative">
                            <select name="user_id" id="user_id"
                                class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 appearance-none bg-white @error('user_id') border-red-500 @enderror">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <select name="priority" id="priority"
                                    class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 appearance-none bg-white @error('priority') border-red-500 @enderror"
                                    required>
                                    <option value="low"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <select name="status" id="status"
                                    class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 appearance-none bg-white @error('status') border-red-500 @enderror"
                                    required>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div>
                            ...
                            <input type="date" name="start_date" id="start_date"
                                class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 @error('start_date') border-red-500 @enderror"
                        ...
                        <div>
                            ...
                            <input type="date" name="deadline" id="deadline"
                                class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 @error('deadline') border-red-500 @enderror"
                    ...
                    <!-- Task Info -->
                    <div class="mt-8 bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-2xl border border-blue-100">
                                ...
                                <ul class="space-y-2">
                                    @foreach ($task->files->take(3) as $file)
                                        <li class="flex items-center justify-between bg-white p-3 rounded-xl shadow-sm">
                                            <span
                                                class="text-sm text-gray-700 truncate flex-1">{{ $file->original_name ?? basename($file->file_path) }}</span>
                                            <span
                                                class="text-xs text-gray-500 ml-2">{{ $file->created_at->diffForHumans() }}</span>
                                        </li>
                                    @endforeach
                                    @if ($task->files->count() > 3)
                                        <li class="text-sm text-gray-500 text-center">... and
                                            {{ $task->files->count() - 3 }} more files</li>
                                    @endif
                                </ul>
                            @else
                                ...
                                <div class="space-y-2">
                                    ...
                                    @if ($task->comments->count() > 3)
                                        <p class="text-sm text-gray-500 text-center">... and
                                            {{ $task->comments->count() - 3 }} more comments</p>
                                    @endif
                                </div>
                            @else
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            form.addEventListener('submit', function(e) {
                ...
                submitBtn.innerHTML =
                    '<svg class="animate-spin h-5 w-5 mr-2 inline-block" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>Updating...';
            });
