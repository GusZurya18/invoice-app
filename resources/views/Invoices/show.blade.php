...
@section('content')
    <div class="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            <h1 class="text-2xl font-bold text-gray-900">{{ __('invoice.invoice') }} {{ $invoice->code }}
                            </h1>
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <div class="relative inline-block">
                            <button onclick="toggleLanguageMenu(event)" id="langButton"
                                ...
                                <svg class="w-4 h-4 ml-2 transition-transform duration-200" id="langChevron" fill="none"
                        ...
                        <div class="relative inline-block">
                            <button onclick="togglePdfMenu(event)" id="pdfButton"
                                ...
                                <svg class="w-4 h-4 ml-1 transition-transform duration-200" id="pdfChevron" fill="none"
                                    stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M19 9l-7 7-7-7"></path>
                                </svg>
        ...
        <div id="langMenu"
            class="hidden fixed w-56 bg-white rounded-xl shadow-2xl border border-gray-200 overflow-hidden transform opacity-0 scale-95 transition-all duration-200"
        ...
        <div id="pdfMenu"
            ...
            <div class="py-1">
                <a href="{{ route('invoices.pdf', ['invoice' => $invoice, 'lang' => 'id']) }}"
                    ...
                    <svg class="w-4 h-4 text-red-600 opacity-0 group-hover:opacity-100 transition-opacity" fill="none"
                        stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 print-container">
            ...
            <div class="bg-white shadow-2xl rounded-2xl overflow-hidden border border-gray-100">
                <!-- Invoice Header -->
                <div
                    class="relative bg-gradient-to-r from-blue-600 via-blue-600 to-blue-800 text-white p-8 invoice-header">
                ...
                <div class="p-8">
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
                        ...
                        <div class="space-y-4">
                            <!-- Status Badge -->
                            <div
                                class="bg-white rounded-xl p-4 border-2 {{ $invoice->status === 'paid' ? 'border-green-200' : ($invoice->status === 'pending' ? 'border-yellow-200' : ($invoice->status === 'draft' ? 'border-gray-200' : 'border-red-200')) }}">
                                ...
                                <div class="flex items-center justify-between mb-3">
                                    <span
                                        class="text-sm font-medium text-gray-600">{{ __('invoice.payment_status') }}</span>
                                    <span
                                        class="px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wider
                                    {{ $invoice->paid_status === 'done'
                                        ? 'bg-green-100 text-green-800'
                            ...
                            <div class="bg-gray-50 rounded-xl p-4 space-y-3">
                                @if ($invoice->start_date)
                                    <div class="flex justify-between items-center">
                                        ...
                                        <span
                                            class="text-sm font-medium">{{ $invoice->start_date->format('d/m/Y') }}</span>
                                    </div>
                                @endif
                                ...
                                @if ($invoice->due_date)
                                    <div class="flex justify-between items-center">
                                        ...
                                        <span
                                            class="text-sm font-medium {{ $invoice->due_date->isPast() && $invoice->paid_status !== 'done' ? 'text-red-600' : '' }}">
                                ...
                                <table class="w-full">
                                    <thead class="bg-gradient-to-r from-gray-50 to-blue-50">
                                        <tr>
                                            ...
                                            <th
                                                class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                                                {{ __('invoice.product') }}</th>
                                            <th
                                                class="px-6 py-4 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">
                                                {{ __('invoice.qty') }}</th>
                                            <th
                                                class="px-6 py-4 text-right text-xs font-semibold text-gray-600 uppercase tracking-wider">
                                                {{ __('invoice.unit_price') }}</th>
                                            <th
                                                class="px-6 py-4 text-right text-xs font-semibold text-gray-600 uppercase tracking-wider">
                                                {{ __('invoice.total') }}</th>
                                        </tr>
                                        ...
                                        @foreach ($invoice->items as $index => $item)
                                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                                <td class="px-6 py-4 text-sm text-gray-900">
                                                    <div
                                                        class="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center text-blue-800 font-semibold">
                                                        {{ $index + 1 }}
                                                    </div>
                                                </td>
                                                ...
                                                <td class="px-6 py-4 text-sm text-gray-900 text-right font-mono">Rp
                                                    {{ number_format($item->unit_price, 0, ',', '.') }}</td>
                                                <td
                                                    class="px-6 py-4 text-sm font-semibold text-gray-900 text-right font-mono">
                            ...
                            <div class="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-200 rounded-xl p-6">
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div class="flex justify-between items-center py-2">
                                        ...
                                        <span class="font-semibold text-gray-900 font-mono">Rp
                                    ...
                                    @if ($invoice->discount_percent > 0)
                                        <div class="flex justify-between items-center py-2">
                                            ...
                                            <span
                                                class="text-blue-600 font-medium">{{ $invoice->discount_percent }}%</span>
                                        </div>

                                        <div class="flex justify-between items-center py-2">
                                            <span class="text-gray-600">Potongan:</span>
                                            <span class="text-red-500 font-medium font-mono">-Rp
                                                {{ number_format($invoice->discount_amount, 0, ',', '.') }}</span>
                                        </div>

                                        <div class="flex justify-between items-center py-2">
                                            <span
                                                class="text-gray-600 font-medium">{{ __('invoice.subtotal_after_discount') }}:</span>
                                            <span class="font-semibold text-gray-900 font-mono">Rp
                                                {{ number_format($invoice->subtotal_after_discount, 0, ',', '.') }}</span>
                                        </div>
                                    @endif
                                    ...
                                    @if ($invoice->tax_rate > 0)
                                        <div class="flex justify-between items-center py-2">
                                            ...
                                            <span
                                                class="text-green-600 font-medium">{{ number_format($invoice->tax_rate, 2) }}%</span>
                                        </div>

                                        <div class="flex justify-between items-center py-2">
                                            <span class="text-gray-600">Biaya Pajak:</span>
                                            <span class="text-green-600 font-medium font-mono">+Rp
                                    ...
                                    <!-- Grand Total -->
                                    <div
                                        class="flex justify-between items-center py-3 bg-white rounded-lg px-4 shadow-sm border border-gray-100">
                                        <span
                                            class="text-lg font-bold text-gray-900">{{ __('invoice.grand_total') }}:</span>
                                        <span class="text-2xl font-bold text-green-600 font-mono">Rp
                                            {{ number_format($invoice->grand_total ?? $invoice->total_amount, 0, ',', '.') }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
